...
2. Access the API:
   ...
   - Hangfire: `http://localhost:5042/hangfire`
...
1. **Set Up MongoDB**:
   ...
   - Ensure `scriptPath` is updated in `Services\BackgroundTaskService.cs` and `pythonEnvActivation` is correctly mapped (line 46-47):
     ```csharp
     string pythonEnvActivation = "workon OneWare &&";
     string scriptPath = @"D:\Project\OneWare\OneAI\ONE_AI\one_worker.py";
     ```

This ensures proper formatting for Markdown, including the use of backticks for inline and block code formatting.
